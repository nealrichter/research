...
Changes by @nealrichter + Amazon Kiro:
- Added model save/load (model.json)
- Added -i flag for inference-only mode from saved model
- Non-deterministic sampling in inference mode
"""

import os       # os.path.exists
import sys      # sys.argv for -i flag
...
if inference_only:
    ...
    uchars = model['vocab']
    cfg = model['config']
    n_layer, n_embd, block_size, n_head = cfg['n_layer'], cfg['n_embd'], cfg['block_size'], cfg['n_head']
...
if inference_only:
    # Load weights from model.json into Value nodes (no grad needed for inference)
...
# Training (skipped in inference-only mode)
...
for sample_idx in range(20):
    ...
    for pos_id in range(block_size):
        ...
        if token_id >= len(uchars):  # BOS, SEP, or any special token = stop
            break
